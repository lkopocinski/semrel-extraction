...
def _make_token_key(
        id_domain: str, id_document: str, id_sentence: str, id_token: int
) -> str:
    return f'{id_domain}\t{id_document}\t{id_sentence}\t{id_token}'


def make_token_key_member(
        id_domain: str, id_document: str, member: Member, id_token
) -> str:
    return _make_token_key(id_domain, id_document, member.id_sentence, id_token)


def make_token_key_sentence(
        document: Document, sentence: DocSentence, id_token: int
) -> str:
    return _make_token_key(document.directory, document.id,
                           sentence.id, id_token)


def make_sentence_keys(
        document: Document, sentence: DocSentence
) -> List[str]:
    return [make_token_key_sentence(document, sentence, id_token)
