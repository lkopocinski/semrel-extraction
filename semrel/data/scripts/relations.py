...
import semrel.data.scripts.constant as constant
from semrel.data.scripts.corpus import Document
from semrel.data.scripts.entities import Relation, Member
from semrel.data.scripts.maps import VectorsMap
from semrel.data.scripts.max_pool import max_pool_relation_vectors
from semrel.data.scripts.utils.keys import make_token_key_member, \
    make_relation_key
...
class RelationsLoader:
    ...
    def relations(self):
        with self._relations_path_csv.open(
            ...
            for line_dict in reader_csv:
                label = line_dict[constant.LABEL]
                id_domain = line_dict[constant.DOMAIN]
    ...
    def _parse_relation(relation_dict: Dict) -> Relation:
        id_document = relation_dict[constant.DOCUMENT]
        member_from = Member(
            id_sentence=relation_dict[constant.SENTENCE_FROM],
            lemma=relation_dict[constant.LEMMA_FROM],
            channel=relation_dict[constant.CHANNEL_FROM],
            is_named_entity=eval(relation_dict[constant.NAMED_ENTITY_FROM]),
            indices=eval(relation_dict[constant.INDICES_FROM]),
            context=eval(relation_dict[constant.CONTEXT_FROM])
        )
        member_to = Member(
            id_sentence=relation_dict[constant.SENTENCE_TO],
            lemma=relation_dict[constant.LEMMA_TO],
            channel=relation_dict[constant.CHANNEL_TO],
            is_named_entity=eval(relation_dict[constant.NAMED_ENTITY_TO]),
            indices=eval(relation_dict[constant.INDICES_TO]),
            context=eval(relation_dict[constant.CONTEXT_TO])
        )
